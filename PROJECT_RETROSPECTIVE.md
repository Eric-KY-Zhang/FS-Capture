...
- 日股：新增 EDINET 插件。v0.9 只完成 EDINET API key 路径；真正的无 Key 公网爬虫在 v1.0 addendum 补齐。
...
v0.9 批次 8 实施 JP 时只做了 API key 路径，`edinet_web.py` 是占位 stub。v1.0 Reviewer 阶段发现并补做真公网爬虫：

- 数据源：EDINET `WEEE0030.aspx` 公网搜索页。Spike 确认它是 GeneXus 页面，不是 ASP.NET WebForms；纯 httpx 复现 3 段加密 `GXAjaxRequest` 成本过高。
- 搜索实现：Playwright 复用 `pdf_renderer._ensure_state()` thread-local pool，填写 `W0018vD_KEYWORD` / `W0018vD_KIKAN` / `W0018vCHKSYORUI1` 后捕获 `AV125W_RESULT_LIST_JSON`。
- PDF 下载：结果行里的 `SHORUI_KANRI_NO` 可直接拼 `https://disclosure2dl.edinet-fsa.go.jp/searchdocument/pdf/{doc_id}.pdf`，无需 cookie、token 或 referrer；下载仍走 `default_client(source="edinet_web")` + `stream_to_file`。
- 限流：`edinet_web = 1.0` QPS，API mode 保持 `edinet = 2.0` QPS；EDINET Subscription-Key 变成可选加速器。
- 名称解析：无 Key 时通过公网搜索结果拿 `filer_name` 与 `EDINET_CD`，有 Key 时保留原 365 天 API 枚举。
