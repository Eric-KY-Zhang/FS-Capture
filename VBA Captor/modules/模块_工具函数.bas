...
' --------- Phase 4j Step 3: 跨市场 BS/IS/CF P3 分组 (mapped 上区 + 未映射下区) ---------
Public Sub BuildCrossMarketStatementSheet(ByVal statementKind As String)
    Dim targetSheet As String, statementLabel As String, mappingCode As String
    Select Case UCase$(Trim$(statementKind))
        Case "BALANCESHEET"
            targetSheet = "跨市场_资产负债表": statementLabel = "资产负债表": mappingCode = "BS"
        Case "INCOME"
            targetSheet = "跨市场_利润表": statementLabel = "利润表": mappingCode = "IS"
        Case "CASHFLOW"
            targetSheet = "跨市场_现金流量表": statementLabel = "现金流量表": mappingCode = "CF"
        Case Else
    ...
    Dim mapping As Object: Set mapping = LoadCrossMarketMapping(mappingCode)
    If mapping Is Nothing Then
        BuildCrossMarketStatementSheetP2 statementKind
        On Error Resume Next
        If Not wsTarget.Range("A1").Comment Is Nothing Then wsTarget.Range("A1").Comment.Delete
        wsTarget.Range("A1").AddComment "未找到『字段映射』sheet,本表临时按旧版字段并集方式展示。请重新安装模板生成字段映射后,再重建跨市场对比。"
        wsTarget.Range("A1").Comment.Shape.TextFrame.AutoSize = True
        On Error GoTo 0
        Exit Sub
    End If
    ...
    If mapping.Count = 0 Then
        ...
        wsTarget.Range("B1").Value = "指标名称"
        wsTarget.Range("A3").Value = "请先在『字段映射』sheet 录入 " & mappingCode & " mapping 关系"
        Application.ScreenUpdating = True
        Exit Sub
    End If
    ...
    Dim collCompanies As Collection: Set collCompanies = New Collection
    Dim markets As Variant: markets = Array("A", "US", "HK", "KR")
    Dim marketRows As Object: Set marketRows = CreateObject("Scripting.Dictionary")
    Dim marketMapped As Object: Set marketMapped = CreateObject("Scripting.Dictionary")
    Dim m As Variant
    For Each m In markets
        Dim sourceSheet As String: sourceSheet = MarketStatementSheetName(CStr(m), statementLabel)
        Dim tempRows As Collection: Set tempRows = New Collection
        Dim tempDict As Object: Set tempDict = CreateObject("Scripting.Dictionary")
        If WorksheetExists(sourceSheet) Then
            CollectCompaniesFromIndicatorSheet ThisWorkbook.Sheets(sourceSheet), CStr(m), collCompanies
            CollectRowsFromStatementSheet ThisWorkbook.Sheets(sourceSheet), tempRows, tempDict
        End If
        marketRows.Add CStr(m), tempRows
        marketMapped.Add CStr(m), CreateObject("Scripting.Dictionary")
    Next m
    ...
    Dim stdName As Variant, mapArr As Variant, idx As Long
    For Each stdName In mapping.Keys
        mapArr = mapping(stdName)
        For idx = 0 To 3
            If Len(Trim$(CStr(mapArr(idx)))) > 0 Then
                Dim mappedForMarket As Object
                Set mappedForMarket = marketMapped(MarketCodeByIndex(idx))
                mappedForMarket(Trim$(CStr(mapArr(idx)))) = True
            End If
        Next idx
    Next stdName
    ...
    With wsTarget.Range("A1:B1")
        .Font.Name = "微软雅黑": .Font.Size = 11: .Font.Bold = True
        .Font.Color = RGB(255, 255, 255): .Interior.Color = RGB(31, 56, 100)
        .HorizontalAlignment = xlCenter: .VerticalAlignment = xlCenter
    End With
    ...
    Dim mappedWritten As Long: mappedWritten = 0
    For Each stdName In mapping.Keys
        mapArr = mapping(stdName)
        If CrossMarketMappingHasAnySource(collCompanies, mapArr) Then
            WriteCrossMarketMappedRow wsTarget, rowNum, CStr(stdName), mapArr, collCompanies
            mappedWritten = mappedWritten + 1
            rowNum = rowNum + 1
        End If
    Next stdName

    Dim sepRow As Long: sepRow = rowNum
    wsTarget.Range(wsTarget.Cells(sepRow, 1), wsTarget.Cells(sepRow, lastCol)).Merge
    wsTarget.Cells(sepRow, 1).Value = "未映射字段(按市场保留,可展开查看)"
    With wsTarget.Range(wsTarget.Cells(sepRow, 1), wsTarget.Cells(sepRow, lastCol))
        .Font.Name = "微软雅黑": .Font.Size = 11: .Font.Bold = True
        .Font.Color = RGB(64, 64, 64): .Interior.Color = RGB(217, 217, 217)
        .HorizontalAlignment = xlLeft: .VerticalAlignment = xlCenter
    End With
    rowNum = rowNum + 1

    For Each m In markets
        Dim rowsForMarket As Collection: Set rowsForMarket = marketRows(CStr(m))
        Set mappedForMarket = marketMapped(CStr(m))
        rowNum = WriteCrossMarketUnmappedBlock(wsTarget, rowNum, CStr(m), rowsForMarket, _
                                               mappedForMarket, collCompanies, lastCol)
    Next m

    Dim lastRow As Long: lastRow = rowNum - 1
    If lastRow < sepRow Then lastRow = sepRow

    wsTarget.Columns("A").ColumnWidth = 10
    wsTarget.Columns("B").ColumnWidth = 24
    Dim col As Long
    For col = 3 To lastCol
        wsTarget.Columns(col).ColumnWidth = 15.875
    Next col
    wsTarget.Rows(1).RowHeight = 24
    wsTarget.Rows(2).RowHeight = 22
    wsTarget.Range(wsTarget.Cells(3, 1), wsTarget.Cells(lastRow, lastCol)).Font.Name = "微软雅黑"
    wsTarget.Range(wsTarget.Cells(3, 1), wsTarget.Cells(lastRow, lastCol)).Font.Size = 11
    wsTarget.Range(wsTarget.Cells(3, 3), wsTarget.Cells(lastRow, lastCol)).HorizontalAlignment = xlRight
    wsTarget.Range(wsTarget.Cells(3, 1), wsTarget.Cells(lastRow, 2)).HorizontalAlignment = xlLeft

    If mappedWritten > 0 Then
        With wsTarget.Range(wsTarget.Cells(3, 1), wsTarget.Cells(2 + mappedWritten, lastCol))
            .Interior.Color = RGB(255, 255, 255)
        End With
    End If

    Call SetBorderLine(wsTarget.Range(wsTarget.Cells(1, 1), wsTarget.Cells(lastRow, lastCol)))

    On Error Resume Next
    wsTarget.Outline.SummaryRow = xlSummaryAbove
    wsTarget.Outline.ShowLevels RowLevels:=1
    On Error GoTo 0
    ...
    Dim displayMode As String: displayMode = ReadDisplayCurrency()
    Dim commentText As String
    commentText = "本表合并 A股 / 美股 / 港股 / 韩股 4 个市场的" & statementLabel & "数据。" & vbCrLf & _
                  "字段对齐方式:上区按『字段映射』sheet 统一行项目(已命中 " & mappedWritten & " 项),下区保留各市场独有字段并默认折叠。" & vbCrLf & _
                  "如需调整口径,请编辑『字段映射』sheet 后点击样本池橙色『一键跨市场对比』。" & vbCrLf & _
                  "当前显示币种: " & displayMode
    wsTarget.Range("A1").AddComment commentText
    wsTarget.Range("A1").Comment.Shape.TextFrame.AutoSize = True
...
Private Sub BuildCrossMarketStatementSheetP2(ByVal statementKind As String)
...
Private Function LoadCrossMarketMapping(ByVal mappingCode As String) As Object
    Dim ws As Worksheet
    On Error Resume Next
    Set ws = ThisWorkbook.Sheets("字段映射")
    On Error GoTo 0
    If ws Is Nothing Then
        Set LoadCrossMarketMapping = Nothing
        Exit Function
    End If

    Dim lastRow As Long: lastRow = ws.Cells(ws.Rows.Count, 2).End(xlUp).Row
    Dim maxRows As Long: maxRows = Application.Max(1, lastRow - 1)
    Dim orders() As Double, names() As String, vals() As Variant
    ReDim orders(1 To maxRows)
    ReDim names(1 To maxRows)
    ReDim vals(1 To maxRows)

    Dim n As Long: n = 0
    Dim r As Long
    For r = 2 To lastRow
        If UCase$(Trim$(CStr(ws.Cells(r, 1).Value))) = UCase$(Trim$(mappingCode)) Then
            Dim stdName As String: stdName = Trim$(CStr(ws.Cells(r, 2).Value))
            If Len(stdName) > 0 Then
                n = n + 1
                If IsNumeric(ws.Cells(r, 7).Value) Then
                    orders(n) = CDbl(ws.Cells(r, 7).Value)
                Else
                    orders(n) = n
                End If
                names(n) = stdName
                vals(n) = Array(Trim$(CStr(ws.Cells(r, 3).Value)), _
                                Trim$(CStr(ws.Cells(r, 4).Value)), _
                                Trim$(CStr(ws.Cells(r, 5).Value)), _
                                Trim$(CStr(ws.Cells(r, 6).Value)))
            End If
        End If
    Next r
    ...
    For i = 1 To n - 1
        For j = i + 1 To n
            If orders(j) < orders(i) Then
                Dim tmpOrder As Double: tmpOrder = orders(i)
                orders(i) = orders(j): orders(j) = tmpOrder
                Dim tmpName As String: tmpName = names(i)
                names(i) = names(j): names(j) = tmpName
                Dim tmpVal As Variant: tmpVal = vals(i)
                vals(i) = vals(j): vals(j) = tmpVal
            End If
        Next j
    Next i
    ...
    Dim dict As Object: Set dict = CreateObject("Scripting.Dictionary")
    For i = 1 To n
        If Not dict.Exists(names(i)) Then dict.Add names(i), vals(i)
    Next i
    Set LoadCrossMarketMapping = dict
End Function


Private Function MarketCodeByIndex(ByVal idx As Long) As String
    Select Case idx
        Case 0: MarketCodeByIndex = "A"
        Case 1: MarketCodeByIndex = "US"
        Case 2: MarketCodeByIndex = "HK"
        Case 3: MarketCodeByIndex = "KR"
    End Select
End Function


Private Function MarketIndexForCode(ByVal market As String) As Long
    Select Case UCase$(Trim$(market))
        Case "A": MarketIndexForCode = 0
        Case "US": MarketIndexForCode = 1
        Case "HK": MarketIndexForCode = 2
        Case "KR": MarketIndexForCode = 3
        Case Else: MarketIndexForCode = -1
    End Select
End Function


Private Function MarketDisplayName(ByVal market As String) As String
    Select Case UCase$(Trim$(market))
        Case "A": MarketDisplayName = "A股"
        Case "US": MarketDisplayName = "美股"
        Case "HK": MarketDisplayName = "港股"
        Case "KR": MarketDisplayName = "韩股"
        Case Else: MarketDisplayName = market
    End Select
End Function


Private Function MarketGroupColor(ByVal market As String) As Long
    Select Case UCase$(Trim$(market))
        Case "A": MarketGroupColor = RGB(217, 225, 242)
        Case "US": MarketGroupColor = RGB(244, 204, 204)
        Case "HK": MarketGroupColor = RGB(226, 239, 218)
        Case "KR": MarketGroupColor = RGB(226, 218, 239)
        Case Else: MarketGroupColor = RGB(242, 242, 242)
    End Select
End Function


Private Function CrossMarketMappingHasAnySource(ByVal collCompanies As Collection, ByVal mapArr As Variant) As Boolean
    Dim i As Long
    For i = 1 To collCompanies.Count
        Dim entry As Variant: entry = collCompanies(i)
        Dim mkt As String: mkt = CStr(entry(0))
        Dim idx As Long: idx = MarketIndexForCode(mkt)
        If idx >= 0 Then
            Dim sourceLabel As String: sourceLabel = Trim$(CStr(mapArr(idx)))
            If Len(sourceLabel) > 0 Then
                If FindStatementRowByLabel(ThisWorkbook.Sheets(CStr(entry(3))), sourceLabel) > 0 Then
                    CrossMarketMappingHasAnySource = True
                    Exit Function
                End If
            End If
        End If
...
Private Sub WriteCrossMarketMappedRow(ByVal wsTarget As Worksheet, ByVal rowNum As Long, _
                                      ByVal stdName As String, ByVal mapArr As Variant, _
                                      ByVal collCompanies As Collection)
    wsTarget.Cells(rowNum, 1).Value = "映射字段"
    wsTarget.Cells(rowNum, 2).Value = stdName

    Dim writeCol As Long: writeCol = 3
    Dim i As Long, j As Long
    For i = 1 To collCompanies.Count
        Dim entry As Variant: entry = collCompanies(i)
        Dim idx As Long: idx = MarketIndexForCode(CStr(entry(0)))
        Dim sourceLabel As String
        If idx >= 0 Then sourceLabel = Trim$(CStr(mapArr(idx)))
        Dim srcSheet As String: srcSheet = CStr(entry(3))
        Dim srcStartCol As Long: srcStartCol = CLng(entry(4))
        Dim periodCount As Long: periodCount = CLng(entry(5))
        Dim srcRow As Long: srcRow = 0
        If Len(sourceLabel) > 0 Then srcRow = FindStatementRowByLabel(ThisWorkbook.Sheets(srcSheet), sourceLabel)
...
Private Function WriteCrossMarketUnmappedBlock(ByVal wsTarget As Worksheet, ByVal startRow As Long, _
                                               ByVal market As String, ByVal rows As Collection, _
                                               ByVal mappedDict As Object, ByVal collCompanies As Collection, _
                                               ByVal lastCol As Long) As Long
    Dim hasRows As Boolean: hasRows = False
    Dim k As Long
    For k = 1 To rows.Count
        Dim rowInfo As Variant: rowInfo = rows(k)
        Dim itemText As String: itemText = Trim$(CStr(rowInfo(1)))
        If Len(itemText) > 0 And Not mappedDict.Exists(itemText) Then
            hasRows = True
            Exit For
        End If
    Next k
    If Not hasRows Then
        WriteCrossMarketUnmappedBlock = startRow
        Exit Function
    End If

    Dim headerRow As Long: headerRow = startRow
    wsTarget.Cells(headerRow, 1).Value = MarketDisplayName(market)
    wsTarget.Cells(headerRow, 2).Value = MarketDisplayName(market) & "独有字段"
    With wsTarget.Range(wsTarget.Cells(headerRow, 1), wsTarget.Cells(headerRow, lastCol))
        .Font.Name = "微软雅黑": .Font.Size = 11: .Font.Bold = True
        .Interior.Color = MarketGroupColor(market)
        .HorizontalAlignment = xlLeft: .VerticalAlignment = xlCenter
    End With

    Dim rowNum As Long: rowNum = headerRow + 1
    Dim groupStart As Long: groupStart = rowNum
    For k = 1 To rows.Count
        rowInfo = rows(k)
        itemText = Trim$(CStr(rowInfo(1)))
        If Len(itemText) > 0 And Not mappedDict.Exists(itemText) Then
            wsTarget.Cells(rowNum, 1).Value = Trim$(CStr(rowInfo(0)))
            wsTarget.Cells(rowNum, 2).Value = itemText
            WriteCrossMarketUnmappedFormulaRow wsTarget, rowNum, market, itemText, collCompanies
            rowNum = rowNum + 1
        End If
    Next k

    If rowNum > groupStart Then
        On Error Resume Next
        wsTarget.Rows(CStr(groupStart) & ":" & CStr(rowNum - 1)).Group
        On Error GoTo 0
    End If
    WriteCrossMarketUnmappedBlock = rowNum
End Function


Private Sub WriteCrossMarketUnmappedFormulaRow(ByVal wsTarget As Worksheet, ByVal rowNum As Long, _
                                               ByVal market As String, ByVal itemText As String, _
                                               ByVal collCompanies As Collection)
    Dim writeCol As Long: writeCol = 3
    Dim i As Long, j As Long
    For i = 1 To collCompanies.Count
        ...
        Dim periodCount As Long: periodCount = CLng(entry(5))
        Dim srcRow As Long: srcRow = 0
        If UCase$(CStr(entry(0))) = UCase$(market) Then
            srcRow = FindStatementRowByLabel(ThisWorkbook.Sheets(srcSheet), itemText)
        End If
        For j = 1 To periodCount
            If srcRow > 0 Then
                wsTarget.Cells(rowNum, writeCol).Formula = "='" & srcSheet & "'!" & _
...
Public Sub BuildAllCrossMarketSheets()
    On Error Resume Next
    ThisWorkbook.Sheets("跨市场_资产负债表").Visible = -1
    ThisWorkbook.Sheets("跨市场_利润表").Visible = -1
    ThisWorkbook.Sheets("跨市场_现金流量表").Visible = -1
