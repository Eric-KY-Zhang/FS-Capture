...
1. 在 `A2` 选择年度,在 `A4` 选择季度;年度留空时,工具尽量取可取得的最新报告期。
2. 如需港股或部分中概美股,在 `B5` 填入雪球登录凭证;在 `B6` 选择 `原币` 或 `统一RMB`。
3. 第 11 行起按市场录入公司:`A:B` 为 A股,`E:F` 为美股,`I:J` 为港股,`M:N` 为韩股。
4. 点击对应市场的一键按钮。抓数完成后,对应市场的 4 张正式报表会自动显示。
5. 点击橙色 `一键跨市场对比`,生成 4 张跨市场对比表。
6. 如需调整跨市场行项目口径,编辑「字段映射」sheet 后再次点击橙色按钮重建。

顶部按钮区:

- `一键 A 股 / 一键 美股 / 一键 港股 / 一键 韩股`: 只更新对应市场 4 张表。
- `一键跨市场对比`: 按「字段映射」sheet 重建跨市场资产负债表、利润表、现金流量表和指标表。
- `一键全抓 4 市场`: 顺序更新 16 张正式表,并刷新跨市场对比。
...
- `字段映射`: 定义跨市场 BS/IS/CF 的统一行项目。用户可按审计口径调整映射后重建对比表。
- `跨市场_资产负债表` / `跨市场_利润表` / `跨市场_现金流量表`: 4 市场分表的公式合并视图,横向铺公司×报告期;上区按字段映射统一行项目,下区保留各市场独有字段并默认折叠。
