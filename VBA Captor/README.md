...
|---|---|---|
| A股 | 已支持 | 新浪财经 |
| 美股 | 已支持 | SEC EDGAR companyfacts; 20-F/中概股必要时 fallback 到雪球;O6 可启用 stockanalysis 中概备用路径 |
| 港股 | 已支持 | 雪球 HK API |
...
- `O6`: 中概美股 stockanalysis fallback 开关,默认 `关`;仅在 EDGAR + 雪球都失败时,对 BABA/JD/PDD 启用备用路径
- 第 11 行起按市场录入公司:
  - `A:B`: A股代码 / 简称
  - `E:F`: 美股代码 / 简称
  - `I:J`: 港股代码 / 简称
...
- 顶部按钮:
  ...
  - `S` 列跨市场按钮:`一键跨市场对比`、`切换跨市场 tab 显隐`
  - `Q` 列保留主操作 / 显示 / 工具按钮:`一键全抓 4 市场`、`切换所有市场 tab 显隐`、`清空 HTTP 缓存`
...
- `跨市场_指标表`: 4 张分市场指标表的合并视图,横向铺公司×报告期,18 项标准指标。每次跑数后点『一键跨市场对比』按钮,或使用『一键全抓 4 市场』自动刷新。
...
- `O6` stockanalysis 中概美股 fallback 默认关闭;当前只验证 BABA/JD/PDD,其他中概股可能字段不齐。
