...
|---|---|---|
| A股 | 已支持 | 新浪财经 |
| 美股 | 已支持 | SEC EDGAR companyfacts; 20-F/中概股必要时 fallback 到雪球;B8 可启用 stockanalysis 中概备用路径 |
| 港股 | 已支持 | 雪球 HK API |
...
- `B6`: 显示币种,可选 `原币` / `统一RMB`;已写表数据会随 B6 立即按本地汇率公式切换显示
- `B8`: 中概美股 stockanalysis fallback 开关,位于 cookie / currency 配置区下方(`B7` 标签 + `B8` 开关),默认 `关`;仅在 EDGAR + 雪球都失败时,对 BABA/JD/PDD 启用备用路径
- 第 11 行起按市场录入公司:
  - `A:B`: A股代码 / 简称
  - `E:F`: 美股代码 / 简称
  - `I:J`: 港股代码 / 简称
...
- 顶部按钮:
  ...
  - 合表按钮统一在 `S` 列分组:`合并 4 张跨市场表` 以及 `指标表 / 资产负债表 / 利润表 / 现金流量表` 单独刷新
  - `Q` 列保留主操作 / 显示 / 工具按钮:`一键全抓 4 市场`、`切换所有分市场 tabs 显隐`、`清空缓存`
...
`美股_抓取诊断`、`港股_抓取诊断`、`韩股_抓取诊断` 默认隐藏,需要排查时可在 Excel 里右键任意 sheet tab → 取消隐藏。样本池也提供分市场和全局 tab 显隐按钮。
...
- `原币` 模式下按各市场原口径显示;`统一RMB` 模式下,非 RMB 报告币种会按『汇率』sheet 缓存汇率换算显示
...
- `跨市场_资产负债表` / `跨市场_利润表` / `跨市场_现金流量表`: 4 市场分表的公式合并视图,横向铺公司×报告期;行项目使用当前 4 市场分表的并集顺序保留,避免跨语言字段名差异造成数据丢失。
- `跨市场_指标表`: 4 张分市场指标表的合并视图,横向铺公司×报告期,18 项标准指标。每次跑数后点『合并跨市场指标表』或『合并 4 张跨市场表』按钮,或使用『一键全抓 4 市场』自动刷新。
...
- `B8` stockanalysis 中概美股 fallback 默认关闭;当前只验证 BABA/JD/PDD,其他中概股可能字段不齐。
